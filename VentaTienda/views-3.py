...
def crear_control_diario(request):
    ...
    if request.method == 'POST':
        if existe_control:
            messages.error(request, 'Ya existe un control diario para el día de hoy.')
        ...
        if form.is_valid():
            ...
            # Procesar datos JSON de las tablas
            ...
            for producto in ControlDiarioForm.PRODUCTOS_SALDOS:
                ...
                productos_saldos[producto] = {
                    ...
                    'final': float(request.POST.get(f'saldo_{key}_final', 0) or 0),
                }
            control.productos_saldos = productos_saldos
            
            # Tabla 2: Venta por Producto (con precio unitario)
            venta_productos = {}
            ...
            for producto in ControlDiarioForm.PRODUCTOS_VENTA:
                ...
                total = float(request.POST.get(f'venta_{key}_total', cantidad * precio) or cantidad * precio)
            ...
            gastos = []
            i = 0
            while f'gasto_descripcion_{i}' in request.POST:
                descripcion = request.POST.get(f'gasto_descripcion_{i}', '').strip()
                if descripcion:
                    gastos.append({
                        'descripcion': descripcion,
                        'no_factura': request.POST.get(f'gasto_factura_{i}', ''),
                        'cantidad': float(request.POST.get(f'gasto_cantidad_{i}', 0) or 0)
                    })
                i += 1
